...
static void reset(struct db_main *db)
{
	if (!program[gpu_id]) {
		...
		opencl_init("$JOHN/opencl/gost12256hash_kernel.cl", gpu_id, build_opts);

		init_kernel = clCreateKernel(program[gpu_id], "gost12256init", &ret_code);
		HANDLE_CLERROR(ret_code, "Error creating kernel");

		crypt_kernel = clCreateKernel(program[gpu_id], "gost12256loop", &ret_code);
		HANDLE_CLERROR(ret_code, "Error creating kernel");

		final_kernel = clCreateKernel(program[gpu_id], "gost12256final", &ret_code);
		HANDLE_CLERROR(ret_code, "Error creating kernel");
	}
